...
export default function EditLinkModal({ link, onClose, links, setLinks }) {
  ...
  async function handleUpdate() {
    ...
    try {
      ...
      console.error(t("editLink.updateFailed"), error);
    }
  }

  return (
      <div className="fixed inset-0 bg-black/40 p-5 flex items-center justify-center z-50">
          ...
          <div className="flex flex-col gap-5">
            ...
            <div>
              <label className="block text-gray-700 text-sm font-medium mb-1.5">
                {t("editLink.labels.name")}
              </label>
              <input
                  type="text"
                  placeholder={t("editLink.placeholders.name")}
                  value={linkName}
            ...
            <div>
              <label className="block text-gray-700 text-sm font-medium mb-1.5">
                {t("editLink.labels.url")}
              </label>
              <input
                  type="text"
                  placeholder={t("editLink.placeholders.url")}
                  value={linkUrl}
            ...
            <div className="relative">
              <label className="block text-gray-700 text-sm font-medium mb-1.5">
                {t("editLink.labels.emoji")}
              </label>
              <input
                  type="text"
                  placeholder={t("editLink.placeholders.emoji")}
                  value={linkEmoji}
              ...
              <div className="flex flex-wrap gap-2 border-2 border-gray-200 rounded-lg p-2.5 bg-white/50 focus-within:border-[#0c8f63] focus-within:ring-2 focus-within:ring-[#0c8f63]/30 transition-all duration-300">
                ...
                {linkTags.length < 3 && (
                    <input
                        ...
                        placeholder={t("editLink.placeholders.tags")}
                    />
                )}
              </div>
            </div>
