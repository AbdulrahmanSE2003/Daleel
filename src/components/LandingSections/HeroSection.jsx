...
function useSplashScreen(delay) {
...
function HeroSection() {
  ...
  const showSplash = useSplashScreen(1800); // duration for splash

  return (
      <section className="relative min-h-screen bg-gradient-to-br from-[#0c8f63]/5 via-white to-[#0c8f63]/10 overflow-hidden">
        {/* Splash Screen */}
        {showSplash && (
            <div className="fixed inset-0 z-50 flex flex-col justify-center items-center bg-gradient-to-b from-white to-[#0c8f63]/30 backdrop-blur-sm transition-opacity duration-600 ease-in-out">
            ...
            <span className="relative">
              ...
              <h1 className="text-4xl md:text-5xl font-extrabold mt-5 text-emerald-800 tracking-tight">
                Daleel
              </h1>
            </div>
        )}
        ...
        >
          ...
          {/* Left Text */}
          <div className="w-full md:w-1/2 flex flex-col justify-center items-start gap-5 md:gap-8 z-10 mt-12 md:mt-0">
            <h5 className="text-2xl sm:text-3xl md:text-4xl font-semibold uppercase tracking-wider text-gray-900 animate-slide-in-left">
              {t("hero_heading")}{" "}
              <span className="text-[#0c8f63] font-bold drop-shadow-md">
              {t("hero_highlight")}
            </span>{" "}
              {t("hero_place")}
            </h5>
